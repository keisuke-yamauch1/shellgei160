# シェル・ワンライナー160本ノック（[#shellgei160](https://twitter.com/hashtag/shellgei160?src=hashtag_click&f=live)）のリポジトリ
...
|-----|--------------------|-------------|----------------|----------|----------|
|p.50|3番目のコードブロック直後の1行|`というように、年月日、自分秒がそれぞれ 8 桁、6 桁で出力されます。` | `というように、年月日、時分秒がそれぞれ 8 桁、6 桁で出力されます。` | [issue57](https://github.com/shellgei/shellgei160/issues/57) |変換ミスと思われます。|
|p.51|別解2|`$ cat log_range.log \| awk '$4" "$5>="[24/Dec/2016 21:00:00]" && $4" "$5<"[25/Dec/2016 03:59:60]"'`|`$ cat log_range.log \| awk '$4" "$5>="[24/Dec/2016 21:00:00]" && $4" "$5<"[25/Dec/2016 03:59:60]" && a == 0;$4" "$5>"[25/Dec/2016 02:59:60]"{a++}'`|[issue56](https://github.com/shellgei/shellgei160/issues/56)|データに3時台のレコードが1つしかなくて不備を見落としていました。|
|p.51|別解2の下の行|（⇒練習1.3.g）。|（⇒練習1.3.g）。`a==0`以降は、3時台のデータを2行以上出力しないための細工です。|同上|同上|
